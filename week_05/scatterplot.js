...
const svg = d3.select("#chart")
    .append("svg")
    .attr("viewBox", [0, 0, width, height]); //change the size based on the screen size

d3.csv('penguins.csv').then(data => {
  
  let x = d3.scaleLinear() // claim x scale
    .domain(d3.extent(data, d => d.body_mass_g)).nice() // data
    .range([margin.left, width - margin.right]); // space

  let y = d3.scaleLinear()
    .domain(d3.extent(data, d => d.flipper_length_mm)).nice() // d3.extent: give the minimum and maximum
