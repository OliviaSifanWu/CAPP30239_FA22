...
d3.csv('long-term-interest-canada.csv').then(data => {
    ...
    svg.append("g")
      ...
      .call(d3.axisBottom(x).tickFormat(d3.timeFormat("%b"))); // remove "2020" from the start of the x-axis

    svg.append("text") // add label to x-axis
      .attr("class", "x-label")
    ...
    svg.append("text") // add label to y-axis
      .attr("class", "y-label")
      .attr("text-anchor", "end")
      .attr("x", -margin.top/2)
      .attr("dx", "-0.5em")
